...
class SVGADrawable(val videoItem: SVGAVideoEntity, val dynamicItem: SVGADynamicEntity): Drawable() {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        if(BuildConfig.DEBUG) {
            Log.i(TAG, "method->draw canvas width: ${canvas?.width},height: ${canvas?.height}")
            Log.i(TAG, "method->draw bound width: ${bounds?.width()},height: ${bounds?.height()}")
        }
        canvas?.let {
...
open class SVGAImageView : ImageView {
    ...
    var cancelAnimatorWhenDetach = true
    ...
    private fun loadAttrs(attrs: AttributeSet) {
        ...
        loops = typedArray.getInt(R.styleable.SVGAImageView_svgLoopCount, 0)
