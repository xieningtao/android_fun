...
public class UploadElkHelper {
    ...
    public static void uploadElkFile(Context context, String rootLogDir, File file) {
        mCreator.getInstance().upload(context, rootLogDir, file);
    }
    ...
    public static void uploadIdleElkFile(Context context, String elkDir) {
        ...
        if (dir.exists()) {//获取当前目录下的所有文件
            ...
            for (File file : files) {
                UploadElkHelper.uploadElkFile(context, elkDir, file);
            }
        }
    }
    ...
    static class UploadElkImpl implements IUpload {
        ...
        @Override
        public void upload(Context context, String rootLogDir, File file) {
            ...
            if (fileName.contains(IDLE_SUFFIX)) {
                // 为正在上传文件重命名
                newFileName = ElkHelper.remarkFile(rootLogDir, file, IDLE_SUFFIX, UPLOADING_SUFFIX);
